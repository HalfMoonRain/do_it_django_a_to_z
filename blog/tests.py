...
class TestView(TestCase):
    def setUp(self):
        ...
        self.post_002 = Post.objects.create(
            title='두 번째 포스트 입니다.',
            content='Second World. We are the world',
    ...
    def category_card_test(self, soup):
        categoried_card = soup.find('div', id='categories-card')
        self.assertIn('Categories', categoried_card.text)
        self.assertIn(f'{self.category_programming} ({self.category_programming.post_set.count()})',
                      categoried_card.text)
        self.assertIn(f'{self.category_music} ({self.category_music.post_set.count()})',
                      categoried_card.text)
        self.assertIn(f'미분류 (1)', categoried_card.text)
